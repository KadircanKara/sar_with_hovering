...
class PathAnimation():
    ...
    def __init__(self, sol):
        ...
        self.sol:PathSolution = sol
        self.x_matrix , self.y_matrix = get_real_paths(self.sol)
    ...
    def create_figure(self):
        ...
        self.drone_animation = [axis.scatter([], [], color=drone_colors[i], label=f'Drone {i}') for i in range(self.sol.info.number_of_nodes)]
        self.connectivity_lines = [plt.plot([], color='k', label=f"Drone {i}") for i in range(self.sol.info.number_of_nodes)]

    def animate(self, frame):
        # DRONE PATHS
        for i in range(self.sol.info.number_of_drones + 1):
        ...
        # CONNECTIVITY LINES
        real_connectivity_matrix = get_real_connectivity_matrix(self.x_matrix, self.y_matrix, self.sol)
        connectivity_matrix = real_connectivity_matrix[frame]
        ...
        # connectivity_matrix = np.zeros((self.sol.info.number_of_nodes,self.sol.info.number_of_nodes))
...
sol = np.load('Results/X/g_8_a_50_n_8_v_2.5_r_2_minv_1_maxv_5_Nt_1_tarPos_12_ptdet_0.99_pfdet_0.01_detTh_0.9_maxIso_0_SolutionObjects.npy',allow_pickle=True)[0][0]
PathAnimation(sol)
